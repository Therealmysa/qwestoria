      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="flex flex-col md:flex-row gap-4 mb-8 dark:bg-black/20 dark:backdrop-blur-xl dark:border dark:border-white/15 bg-white/80 backdrop-blur-md p-6 rounded-2xl shadow-2xl dark:shadow-blue-500/20 transform hover:scale-[1.02] transition-all duration-300">
          <div className="relative flex-grow">
            ...
            <Input placeholder="Rechercher un article..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="pl-10 dark:bg-black/30 dark:border-white/20 dark:text-white dark:placeholder-blue-300/60 border-blue-200 focus:border-blue-400 dark:focus:border-blue-400" />
          </div>
        ...
        {isLoading ? <div className="flex justify-center items-center py-12">
            ...
            {getFilteredAndSortedPosts().map(post => <Card key={post.id} className="group overflow-hidden hover:shadow-2xl transition-all duration-500 dark:bg-black/20 dark:backdrop-blur-xl dark:border dark:border-white/15 bg-white/90 backdrop-blur-md shadow-xl transform hover:scale-[1.05] hover:-translate-y-2 dark:shadow-blue-500/30 dark:hover:shadow-blue-500/40">
                ...
                <CardHeader className="pb-2">
                  ...
                  <CardTitle className="text-xl text-gray-800 dark:text-white hover:text-primary dark:hover:text-blue-400 transition-colors line-clamp-2 cursor-pointer group-hover:text-gradient-modern" onClick={() => handleReadArticle(post.id)}>
                    {post.title}
                  </CardTitle>
                </CardHeader>
                
                <CardContent className="pb-4">
                  {post.summary ? <p className="text-gray-600 dark:text-gray-300 text-sm mb-3 line-clamp-3">
                      {post.summary}
                    </p> : <p className="text-gray-600 dark:text-gray-300 text-sm mb-3 line-clamp-3">
                ...
                <CardFooter className="pt-0 border-t dark:border-white/15 border-blue-100">
                  <Button variant="ghost" className="w-full text-primary dark:text-blue-400 hover:bg-primary/10 dark:hover:bg-blue-500/20 group-hover:bg-gradient-to-r group-hover:from-blue-600/20 group-hover:to-cyan-600/20 transition-all duration-300" onClick={() => handleReadArticle(post.id)}>
                    Lire l'article
                    <Eye className="ml-2 h-4 w-4" />
                  </Button>
                </CardFooter>
              </Card>)}
          </div> : <div className="dark:bg-black/15 dark:backdrop-blur-xl dark:border dark:border-white/10 bg-gray-50/90 backdrop-blur-md rounded-2xl p-12 text-center shadow-2xl dark:shadow-blue-500/20 transform hover:scale-[1.02] transition-all duration-300">
            ...
            {searchQuery && <Button variant="outline" onClick={() => setSearchQuery("")} className="border-primary text-primary hover:bg-primary/10 dark:border-blue-400 dark:text-blue-400 dark:hover:bg-blue-400/20 dark:bg-black/20 dark:backdrop-blur-xl">
